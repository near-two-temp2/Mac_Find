//! 64-bit 字母 bitmask 编码（Phase 1 O(1) 预过滤的核心）。
//!
//! 编码方案（对齐 Cling，见 open-source-analysis.md §3.3）：
//! ```text
//! Bits 0-25:  字母 a-z
//! Bits 26-35: 数字 0-9
//! Bit 36:     '.'（点）
...
//! `entry_mask & combined_mask != combined_mask` 一条指令排除不可能候选。

/// 对单个字节置位，返回该字节对应的 mask 贡献（已经是小写字节）。
#[inline(always)]
...
///
/// 若返回 false，则该 entry 一定不匹配，可安全跳过（无假阴性）。
#[inline(always)]
...
///   - 位置 0；
///   - 前一个字符是分隔符（`/ . - _ 空格`）；
///   - camelCase：前一个是小写、当前是大写（这里都已小写，故退化为字母跟在非字母后）。
