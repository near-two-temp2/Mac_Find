...
///   Bit 37:     '-' (hyphen)
///   Bit 38:     '_' (underscore)
/// Every other byte is ignored (contributes no bit), which is fine: the mask is
/// only ever used as a *necessary* condition for a match, never a sufficient one.
public enum Bitmask {
...
/// ASCII-lowercase a byte (leaves non-A-Z untouched, including UTF-8 continuation
/// bytes which we deliberately do not case-fold).
@inline(__always)
